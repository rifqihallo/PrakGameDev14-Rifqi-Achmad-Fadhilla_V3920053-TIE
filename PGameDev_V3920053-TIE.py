...
class PemutarMusik(Frame):
    def __init__(self,parent):
        ...
        parent.geometry("640x360")
        self.parent = parent

        self.nama = StringVar()
        self.nama.set("Buka File Yang Dipilih")
    ...
    def bukaFile(self):
        tipeFile = [('ogg file', '*.ogg'), ('All files', '*')]
    ...
    def slider(self):
        w1 = Scale(from_=0.00, to=1.0,resolution=0.1, command=self.volume, orient=HORIZONTAL, length=500, label='Atur Volume :',showvalue=0)
        w1.pack()
        w1.set(0.50)
